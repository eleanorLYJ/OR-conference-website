...
export default function SummaryUpload () {
	...
	const handleFileChange = (e) => {
		...
		if (file && file.size <= 10 * 1024 * 1024) { // 限制文件大小為 10MB
		...
		try {
			...
			for (const [index, author] of authors.entries()) {
				if (author.firstName && author.lastName && author.email) {
					const fullName = `${sanitizeInput(author.firstName)} ${sanitizeInput(author.lastName)}`;
					try {
						const response = await fetch('/api/checkOrCreateUser', {
							...
							body: JSON.stringify({ ...sanitizeInput(author), username: fullName }),
						});
			...
			formData.append('summary', sanitizeInput(summary));

			const uploadResponse = await fetch('/api/uploadSummary', {
				method: 'POST',
				headers: {
					'X-CSRF-Token': 'your-csrf-token-here' // 添加 CSRF token
				},
				body: formData,
			});
			...
			<PageBackground>
				<main className={`${GeistSans.className} min-h-screen flex flex-col items-center justify-start`} style={{ paddingTop: '100px' }}>
					...
					<div className="bg-yellow-100 border-l-4 border-yellow-500 text-yellow-700 p-4 mb-4" role="alert">
						...
						<p>For security reasons, please avoid using the following special characters in your inputs or uploaded content:</p>
						<p className="font-mono">&lt; &gt; &amp; &quot; &#39;</p>
						<p>These characters may be automatically removed or escaped.</p>
					</div>
